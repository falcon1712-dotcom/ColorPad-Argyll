...
static int xcal_read_cgats(xcal *p, cgats *tcg, int table, char *filename) {
	...
	char *ftname = "Calibration";
	char buf[100];

	oi = tcg->get_oi(tcg, "CAL");
	...
	/* Grab general information from it */
	if ((ti = tcg->find_kword(tcg, table, "DESCRIPTOR")) >= 0) {
		if ((p->descriptor = strdup(tcg->t[table].kdata[ti])) == NULL) {
			sprintf(p->e.m, "%s file '%s' strdup of DESCRIPTOR failed",ftname,filename);
			return p->e.c = 1;
		}
	}

	if ((ti = tcg->find_kword(tcg, table, "ORIGINATOR")) >= 0) {
		if ((p->originator = strdup(tcg->t[table].kdata[ti])) == NULL) {
			sprintf(p->e.m, "%s file '%s' strdup of ORIGINATOR failed",ftname,filename);
			return p->e.c = 1;
		}
	}
	...
	if ((ti = tcg->find_kword(tcg, table, "DEVICE_CLASS")) < 0) {
		sprintf(p->e.m, "%s file '%s' doesn't contain keyword DEVICE_CLASS",ftname,filename);
		return p->e.c = 1;
	}
	if (strcmp(tcg->t[table].kdata[ti],"INPUT") == 0) {
		...
		sprintf(p->e.m,"%s file '%s' contain unknown DEVICE_CLASS '%s'",
		                                            ftname,filename,tcg->t[table].kdata[ti]);
		return p->e.c = 1;
	}
	...
	if ((ti = tcg->find_kword(tcg, table, "COLOR_REP")) < 0) {
		...
		if (p->devclass != icSigDisplayClass) {
			sprintf(p->e.m, "%s file '%s' doesn't contain keyword COLOR_REP",ftname,filename);
			return p->e.c = 1;
		} 
		warning("\n    *** %s file '%s' doesn't contain keyword COLOR_REP, assuming RGB ***",ftname,filename);
		if ((p->devmask = icx_char2inkmask("RGB") ) == 0) {
			sprintf(p->e.m, "%s file '%s' has unrecognized COLOR_REP '%s'",
			                                          ftname,filename,tcg->t[table].kdata[ti]);
			return p->e.c = 1;
		}
	} else {
	...
	if (tcg->t[table].nsets <= 0) {
		sprintf(p->e.m, "%s file '%s' has too few entries %d",
		                                          ftname,filename,tcg->t[table].nsets);
		return p->e.c = 1;
	}
	...
	if ((spi[0] = tcg->find_field(tcg, table, buf)) < 0) {
		sprintf(p->e.m,"%s file '%s' doesn't contain field '%s'",ftname,filename,buf);
		return p->e.c = 1;
	}
...
static int xcal_read(xcal *p, char *filename) {
	...
	tcg->add_other(tcg, "CAL"); 	/* our special input type is Calibration Table */
...
static int xcal_write_cgats(xcal *p, cgats *tcg) {
	...
	tcg->add_kword(tcg, table, "DESCRIPTOR", p->descriptor != NULL ? p->descriptor : "Argyll Device Calibration Curves",NULL);

	tcg->add_kword(tcg, table, "ORIGINATOR", p->originator != NULL ? p->originator : "Argyll", NULL);
