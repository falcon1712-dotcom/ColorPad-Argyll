...
/* TTBD:
	...
	NOTE that we aren't allowing for device link with show TAC and
	separate/different cal curves for in and out space etc. ....
...
//  fprintf(stderr,"                P = absolute perceptual, S = absolute saturation\n");
	...
	fprintf(stderr," -T             Show total ink limit of device values\n");
...
main(int argc, char *argv[]) {
	...
	double showtac = 0;			/* Show device TAC */
	static char tcalname[MAXNAMEL+1] = ""; /* .cal overide for icc */
    xcal *tcal = NULL;			/* TAC calibration override for icc */
	int intsep = 0;
	...
	for (fa = 1;fa < argc;fa++) {
		...
		if (argv[fa][0] == '-')	{	/* Look for any flags */
			...
			/* Extra flags */
				...
				{
					usage("-Y function not recognised");
				}
			}
	...
	if (dovcgt) {
		...
		/* Read any user supplied cal curves */
		if (tcalname[0] != '\000') {
    		xcal *xc;
		...
		if (verb > 1 && icco != NULL) {
			...
			if (icco->header->deviceClass == icSigLinkClass)
				colorSpace = icco->header->pcs;
			else
				colorSpace = icco->header->colorSpace;
			...
			if (tac <= -1.0)
				...
				printf("B2A TAC = %f, chan max = %s\n",tac,
				    icmPdv(icmCSSig2nchan(colorSpace), chmax));

			/* Should use icxGetLimits() instead ?? */

			/* Use any device calibration curves that are in the 'targ' .ti3 or given to us */
			ctx.cal = tcal;

			/* Or use any 'targ' .ti3 device calibration curves */
			if (ctx.cal == NULL)
				ctx.cal = xiccReadCalTag(icco);

			if (ctx.cal != NULL
			) {
				tac = icco->get_tac(icco, chmax, xiccCalCallback_void, (void *)&ctx);
		
				if (tac > 0.0) {
					printf("B2A Final TAC = %f, chan max = %s\n",tac,
					    icmPdv(icmCSSig2nchan(colorSpace), chmax));
				}
			}
			printf("\n");
		...
		xicco->set_cal_curves(xicco, tcal);
		/* Release our references to them */
		if (tcal)
			tcal->del(tcal);

		/* Get the actual calibration curves being used by the xicc */
		xicco->get_cal_curves(xicco, &tcal);
		/* Take a references to them */
		if (tcal)
			tcal->ref(tcal);
		...
		if (verb > 1) {
			if (ink.tlimit >= 0.0)
				printf("Final Total ink limit is %3.0f%%\n",100.0 * ink.tlimit);
			if (ink.klimit >= 0.0)
				printf("Final Black ink limit is %3.0f%%\n",100.0 * ink.klimit);
		}
...
//fflush(stdout);
		...
		for (;;) {
			...
			char *bp, *nbp;
			int outsloc = 0;
			double in_tac = -1.0, out_tac = -1.0;
			double in_ctac = -1.0, out_ctac = -1.0;
			 ...
			 && ins != icSigHlsData) {
				...
				if (showtac) {
					...
					if (tcal != NULL) {
						in_ctac = 0.0;
						for (i = 0; i < inn; i++)
							in_ctac += tcal->interp_ch(tcal, i, in[i]);
					}
				}
				if (inn == 3 && in_tvenc != 0) {
			 ...
			 && outs != icSigHlsData) {
				...
				if (showtac || (verb > 0 && tlimit >= 0)) {
					...
					if (tcal != NULL) {
						out_ctac = 0.0;
						for (i = 0; i < outn; i++)
							out_ctac += tcal->interp_ch(tcal, i, uout[i]);
					}
				}
				if (scale > 0.0) {
					for (i = 0; i < MAX_CHAN; i++)
			...
			if (verb > 0) {
				...
				if (verb && in_tac >= 0.0)
					fprintf(stdout," TAC %f",in_tac);
				if (verb && in_ctac >= 0.0)
					fprintf(stdout," calTAC %f",in_ctac);
			...
			if (verb && (out_tac > 0.0 || tlimit >= 0))
				printf(" TAC %f",out_tac);
			if (verb && out_ctac >= 0.0)
				fprintf(stdout," calTAC %f",out_ctac);
			if (verb > 0)
