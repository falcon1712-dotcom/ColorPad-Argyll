...
#ifdef COMMPLUS
#include "mpp2.h"		/* model printer profile #2 support */
#endif
...
struct _xicc {
	...
	struct _xcal *cal;	/* Optional device cal, loaded from ICC 'targ', NULL if none */

	/* Public: */
	void                 (*del)(struct _xicc *p);

						/* If the given cal is not NULL, override or set the xicc cal. */
						/* (This takes a reference) */
	void  (*set_cal_curves)(struct _xicc *p,
	                         struct _xcal *cal
                            );

						/* Return the currently set cal */
						/* (It's up to the caller to take a reference if they need to) */
	void  (*get_cal_curves)(struct _xicc *p,
	                         struct _xcal **cal
                            );
...
int set_icxLuLut(
icc                *icco,
int                flags,			/* "create" ICX flags etc. */
icxTransformCreateType icctype,		/* Profile creation type */
