...
#define POWTOL 1e-5			/* [1e-5] Powell optimiser tollerance (was 1e-5 ?) */
#define MAXITS 10000		/* [10000] */
#define USE_CONJGRAD		/* [def] Else use Powell */

#ifdef USE_CONJGRAD
# undef TEST_PDE			/* [und] Check partial derivative calcs */
#endif
...
/* Set parameter counts etc. given p->luord and flags */ 
static void set_counts(mcv *p) {

	p->cvemin = 2;			/* Curve params start here in full param array */

	if (p->noos)
		p->fitmin = 2;		/* Params to fit start here in full param array */
	else
		p->fitmin = 0;		/* Params to fit start here in full param array */

	p->fitcvemin = p->cvemin - p->fitmin;	/* Index of first curve parameter in fit array */

	p->nfitp = p->luord - p->fitmin;	/* Number of params to fit */
...
/* Create a new mcv initiated with the given total parameters */
/* that include offset & scale. */
...
#ifdef TEST_PDE
# pragma message("!!!!!!!!!!!! TEST_PDE is enabled !!!!!!!!!!!")
# define mcv_opt_func mcv_opt_func_
#endif

/* Shaper optimisation function handed to powell() */
...
/* Shaper optimisation function handed to conjgrad() */
...
//printf("~1 del = %f from (trv %f - rv %f)/0.1\n",del,trv,rv);
			printf("######## DV EXCESSIVE at v[%d] is %f should be %f ########\n",i,dv[i],del);
		}
	}
	return rv;
}
...
static void mcv_fit(mcv *p,
	...
	if (p->noos) {
		...
		/* Set offset and scale to reasonable initial values */
		p->pms[0] = min;
		p->pms[1] = max - min;
	}
	p->dra = max - min;
...
#ifdef USE_CONJGRAD
	if (conjgrad(&p->resid, p->nfitp, p->pms+p->fitmin, sa+p->fitmin, POWTOL, MAXITS,
...
#else
	if (powell(&p->resid, p->nfitp, p->pms+p->fitmin, sa+p->fitmin, POWTOL, MAXITS,
...
static double mcv_interp(struct _mcv *p,
	...
	return mcv_interp_p(p, p->pms + p->fitmin, vv);
}
...
static double mcv_inv_interp(struct _mcv *p,
	...
	if (p->noos == 0) {
		/* Undo order 0 & 1, the offset and scale */
		vv -= p->pms[0];
		vv /= p->pms[1];
	}

	for (pix = p->luord-1; pix >= p->cvemin; pix--) {
		int nsec;			/* Number of sections */
		double sec;			/* Section */

		g = -p->pms[pix];	/* Inverse parameter */

		nsec = 1 + pix - p->cvemin;		/* Increase sections for each order */
...
/* Translate a value through the curve */
/* using the given fit parameters */
static double mcv_interp_p(
	mcv *p,
	double *pms,	/* Fit parameters to use - may exclude offset, scale and slope */
	double vv		/* Input value */
) {
	double g;
	int pix;
	...
	for (pix = p->fitcvemin; pix < p->nfitp; pix++) {
		...
		nsec = 1 + pix - p->fitcvemin;		/* Increase sections for each order */
