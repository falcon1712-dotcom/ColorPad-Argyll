...
/*	fprintf(stderr," -r rSMOOTH      RSPL or shaper raw underlying smoothing factor\n"); */
	...
	fprintf(stderr," -Y c:file.cal   Override any calibration info from .ti3\n");
...
int main(int argc, char *argv[]) {
	...
	double tlimit = -1.0;		/* Total ink limit as a % */
	double klimit = -1.0;		/* Black ink limit as a % */
	static char calname[MAXNAMEL+1] = ""; /* calibration file name */
...
//						usage("Unknown argument '%c' to speed flag -q",na[0]);
			...
			/* Extra flags */
				...
				if (na[0] == 'c') {
					...
					strncpy(calname,&na[2],MAXNAMEL); calname[MAXNAMEL] = '\000';
				} else
				{
	...
	icg->add_other(icg, "CAL"); 	/* our special device Calibration s into a .ti3 file..tate */
	...
	if (strcmp(icg->t[0].kdata[dti],"OUTPUT") == 0) {		/* i.e. printer */
		int hascal = 0;					/* nz if there is a .cal in the .ti3 */
		double filimit = - 1.0;			/* file ink limit */
		icxInk ink;						/* Ink parameters */

		/* Check if there is a .cal in the .ti3 or one provided in command line */
		{
			int oi, tab;
		
			if ((oi = icg->get_oi(icg, "CAL")) >= 0) {	/* CAL is registered */
		...
		/* Try and grab appropriate ink limit from the .ti3 file. */
		if (hascal && (ti = icg->find_kword(icg, 0, "FINAL_TOTAL_INK_LIMIT")) >= 0) {
			filimit = atoi(icg->t[0].kdata[ti]);
		} else if ((ti = icg->find_kword(icg, 0, "TOTAL_INK_LIMIT")) >= 0) {
			filimit = atoi(icg->t[0].kdata[ti]);
		}

		/* Check against user set limit */
		if (filimit > 0.0 && filimit <= 400.0) {	/* File has limit */
			if (tlimit > 0.0 && tlimit <= 400.0) {	/* User has specified limit as option */
				if (filimit < tlimit) {
					warning("Ink limit is greater than original chart! (%.0f%% > %.0f%%)",tlimit,filimit);
				}
			} else {								/* Use file limit */
				if (filimit > 80.0)
					tlimit = (int)filimit - 10.0;	/* Rule of thumb - 10% below chart maximum */
				else
					tlimit = (int)filimit;
			}
		}
		...
		/*  but can be edited manually into a .ti3 file...) */
		...
		if (tlimit >= 0 && tlimit < 400.0) {
			if (verb) {
				if (hascal)
					printf("Final total ink limit being used is %.0f%%\n",tlimit);
				else
