...
static int snap_to_gamut(ifarp *s, double *d) {
	...
	for (ss = 0.0, e = 0; e < di; e++)
		ss += d[e];
	dd = fabs(ss - s->uilimit);

	if (dd < 0.0) {
		int j;
		for (j = 0; j < di; j++) 
			d[j] *= s->uilimit/ss;	/* Snap to ink limit */
		rv = 1;
	}
...
double uilimit,			/* (linearized) Ink limit (sum of device coords max) */
...
dump_image(ifarp *s, int pcp) {
	...
	do_plot_vec(minx, maxx, miny, maxy, 
				x1a, y1a, x2a, y2a, nu, DO_WAIT, x3a, y3a, NULL, NULL, 0);
