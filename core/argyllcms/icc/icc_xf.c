...
/* Matrix inversion using Gauss-Jordan elimination with full pivoting. */
/* Return nz on error.  */
static
int invmatrix(
...
/* Clip the given PCS value to be within the 16 bit encodable range */
/* Return nz if clipped */
static int icmClipPCS16(
	icc *icp,
	double out[3],
	double in[3],
	icColorSpaceSignature encSpace,
	icmEncLabVer ver		/* 0 = V2, 2 same as profile */
) {
	int i, clip = 0;
	...
	if (encSpace == icSigXYZData) {
		min[0] = 0.0; max[0] = 1.0 + 32767.0/32768;
		min[1] = 0.0; max[1] = 1.0 + 32767.0/32768;
		min[2] = 0.0; max[2] = 1.0 + 32767.0/32768;

	} else if (encSpace == icSigLabData) {
		/* icmEncV2 */
		min[0] = 0.0;    max[0] = 100.0 + 25500.0/65280.0;
		min[1] = -128.0; max[1] = 127.0 + 255.0/256.0;
		min[2] = -128.0; max[2] = 127.0 + 255.0/256.0;
			...
			if (clutTable2 != NULL) {
				...
				for (e = 0; e < inputChan; e++) {
					...
					*((int *)&iv[-((int)e)-1]) = -ii[e]-1;	/* Trick to supply -ve grid index in iv[] */
												    /* (Note this is only the base for +0.5 center) */
				}
