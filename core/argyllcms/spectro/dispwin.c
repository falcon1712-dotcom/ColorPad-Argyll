...
//		pWcsDisassociateColorProfileFromDevice = WcsDisassociateColorProfileFromDevice;
		pWcsAssociateColorProfileWithDevice = (BOOL (WINAPI*)(A_WCS_PROFILE_MANAGEMENT_SCOPE,PCWSTR,PCWSTR)) WcsAssociateColorProfileWithDevice;
		pWcsDisassociateColorProfileFromDevice = (BOOL (WINAPI*)(A_WCS_PROFILE_MANAGEMENT_SCOPE,PCWSTR,PCWSTR)) WcsDisassociateColorProfileFromDevice;
	 ...
	 && majv == 1 && minv >= 2) {
		...
		for (i = 0; i < dcount; i++) {
			...
			for (jj = xj = j = 0; j < scrnres->ncrtc; j++, xj++) {
				...
				if (crtci->mode == None || crtci->noutput == 0) {
					debugrr2((errout,"Screen %d CRTC %d skipped as it has no mode or no outputs\n",i,xj));
					XRRFreeCrtcInfo(crtci);
					if (has_primary && j == 0)
						xj = -1;			/* Start at beginning */
					continue;
...
static ramdac *dispwin_get_ramdac(dispwin *p) {
	...
	if (p->crtc != 0) {		/* Using Xrandr 1.2 */
		...
		if (nent != p->nent) {
			debugr2((errout,"XRRGetCrtcGammaSize number of entries %d inconsistent with previous value %d\n",nent,p->nent));
			return NULL;
		}
